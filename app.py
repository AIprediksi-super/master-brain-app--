...
st.set_page_config(page_title="Master Brain v16.5: Ultimate Triple Engine", layout="wide")
...
# LOOP DETAIL PARTIKEL BINTANG (ITEM 101-115)
for i in range(30):
    size_p = random.randint(2, 5)
    left_p = random.randint(0, 100)
    dur_p = random.randint(3, 8)
    st.markdown(f'<div class="star" style="width:{size_p}px; height:{size_p}px; left:{left_p}%; animation-duration:{dur_p}s;"></div>', unsafe_allow_html=True)

# --- 4. KONTROL GRADASI (ITEM 116-130) ---
...
# --- 5. FILTER & INPUT (ITEM 131-165) ---
st.markdown("---")

p_filter = st.radio("Saring hasil angka:", ["Semua", "Ganjil", "Genap", "Kecil (0-4)", "Besar (5-9)"], horizontal=True)

manual_input = st.text_area("Tempel Data 4-Digit:", height=150, value=st.session_state.input_area, key="input_area_box")

def reset_paste_only():
    st.session_state.input_area = ""
    st.rerun()

def reset_all_system(): 
    st.session_state.history = []
    st.session_state.input_area = ""
    st.session_state.trigger_run = False
    st.rerun()
...
with b2:
    st.button("🗑️ HAPUS TEKS PASTE", on_click=reset_paste_only)

with b3:
    st.button("🔴 RESET SEMUA DATA", on_click=reset_all_system)

# --- 6. ENGINE ANALISA OTAK UTAMA (ITEM 166-205) ---
def get_predictions(data, f_mode):
    ...
    for i in range(4):
        ...
        # LOGIKA SKORING TERPISAH (9 PARAMETER)
        score_1 = lambda x: col_s.count(int(x)) * 7
        score_2 = lambda x: col_f.count(int(x)) * 0.5
        
        scored = sorted([str(x) for x in range(10)], key=lambda x: score_1(x) + score_2(x), reverse=True)
        ...
        def apply_f(lst_in):
            if f_mode == "Ganjil": return [x for x in lst_in if int(x)%2!=0]
            if f_mode == "Genap": return [x for x in lst_in if int(x)%2==0]
            if f_mode == "Kecil (0-4)": return [x for x in lst_in if int(x)<=4]
            if f_mode == "Besar (5-9)": return [x for x in lst_in if int(x)>=5]
            return lst_in

        # Pilar 3 Hybrid (Ambil satu unggul, satu lawan)
        p_hyb_1 = sink[0] if sink else scored[0]
        p_hyb_2 = lawan[0] if lawan else scored[-1]
        
        res_final["t2"].append({"m": apply_f(sink), "h": [p_hyb_1, p_hyb_2], "c": scored})
        res_final["t1"].append({"m": apply_f(sink[1:]+sink[:1]), "h": [p_hyb_1, p_hyb_2], "c": scored})
        res_final["t3"].append({"m": apply_f(lawan), "h": [p_hyb_1, p_hyb_2], "c": scored})
        
        m_list = sorted(sink if sink else scored, key=lambda x: Counter("".join(data[-lb:])).get(x,0), reverse=True)
        res_final["t4"].append({"m": apply_f(m_list), "h": [p_hyb_1, p_hyb_2], "c": scored})
        
        res_final["t5"].append([x for x in scored if x not in m_list[:4]] + scored)
        
    return res_final

# --- 7. DISPLAY TABEL (ITEM 206-240) ---
if st.session_state.history:
    ...
    if res:
        ...
        def ambil_angka_bersih(d_dict, r_idx, limit, is_t5=False):
            if is_t5: 
        ...
        for title, key, grad, limit in t_confs:
            ...
            for r in range(limit):
                ...
                for c in range(4):
                    html += f"<td style='background:{grad};'>{ambil_angka_bersih(res[key][c], r, limit)}</td>"
                html += "</tr>"
        ...
        for r in range(l_std):
            ...
            for c in range(4):
                d_m = res['t4'][c]
                html_m += f"<td style='background:{gradien_options['Emas Mewah']};'>{ambil_angka_bersih(d_m, r, l_std)}</td>"
            html_m += "</tr>"
        ...
        for r in range(8):
            ...
            for c in range(4):
                html_e += f"<td style='background:#232526; color:red;'>{res['t5'][c][r]}</td>"
            html_e += "</tr>"
